// Spotify API Configuration (Artık tarayıcıda Client Secret tutulmuyor)
let spotifyAccessToken = null

// Deezer API Configuration (İleride yedek olarak kullanılabilir)
...
// ✅ Spotify token alma (Netlify backend üzerinden)
async function getSpotifyToken() {
  try {
    ...
    alert("Spotify bağlantısı kurulamadı. Lütfen tekrar deneyin.")
    return null
  }
}
...
async function loadSongs() {
  ...
  try {
    ...
    for (const genre of selectedGenres) {
      ...
      const searchUrl = `https://api.spotify.com/v1/search?q=genre:${genreName}&type=track&limit=50&market=TR`
    ...
    if (currentSongs.length > 0) {
      ...
      alert("Hiç şarkı bulunamadı. Lütfen farklı türler seçin.")
    }
  } catch (error) {
...
function displayLikedSongs() {
  if (likedSongs.length === 0) {
    likedList.innerHTML = '<p class="empty-message">Henüz beğenilen şarkı yok</p>'
    return
  }
